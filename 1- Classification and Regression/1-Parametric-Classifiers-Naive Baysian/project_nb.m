...
%  
%  Description     : Manual implementation of Naive Bayes classifier 
...
for i=1:100
    ...
    pw=[1/3,1/3,1/3];
    MDL = myNBtrain(datatrain,dtrain,pw);
    %     MDL = fitcnb(datatrain',dtrain,'Prior',pw);
    %% step 3: classify test data using trained classifier
    %     predictedLables = predict(MDL,datatest');
    predictedLables = myNBclassify(MDL,datatest);
    %% step 4: validation
    % Confusion matrix
