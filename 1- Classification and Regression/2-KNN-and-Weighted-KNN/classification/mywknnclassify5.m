function [output] = mywknnclassify5(MDL,datatest)
% Weighted KNN Classifier (from scratch)
...
switch (Distance)
    case 'Euclidean'
        for i=1:size(datatest,2)
            ...
            w(ind0) =1;
            ind1_1= find(D~=0);
           w(ind1_1) =w(ind1_1) ./(sum(w(ind1_1))+eps);
            %% step 5: voting (classify xtest)
            for n=1:numClass
                N(n) = sum(w.*(NN_Labels==Y(n)));
            end
            [mx,indx]= max(N);
            output(i)=Y(indx);
