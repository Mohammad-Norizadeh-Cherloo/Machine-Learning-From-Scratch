function [output] = mywknnregression1(MDL,datatest)
% Weighted K-Nearest Neighbors Regressor (from scratch)
...
switch (Distance)
    case 'Euclidean'
        for i=1:size(datatest,2)
            ...
            NN_outputs= dtrain(ind(1:k));
            D= dis(1:k);
            w= 1./(D+eps);
            w =w ./(sum(w)+eps);
            %% step 5: weigthed averaging (calulate xtests output)
            output(i)= sum(w.*NN_outputs);
